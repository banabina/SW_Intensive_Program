...
int core0_main(void)
{
    ...
    {
        // LPF
        int first_button = P02_IN.U & (0x1 << P1_BIT_LSB_IDX);
        for (uint32 i = 0; i < 100; ++i); // delay

        int second_button = P02_IN.U & (0x1 << P1_BIT_LSB_IDX);
        for (uint32 i = 0; i < 100; ++i); // delay

        int third_button = P02_IN.U & (0x1 << P1_BIT_LSB_IDX);
        for (uint32 i = 0; i < 100; ++i); // delay

        if (first_button == BUTTON_ON && second_button == BUTTON_ON && third_button == BUTTON_ON)
        {
